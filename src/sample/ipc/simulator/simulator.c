...
static void * sim_ipc_video_task( void *instance )
{
    ...
    fp = fopen( H264_FILE, "r" );
    if ( !fp ) {
        LOGE("open file %s error", H264_FILE);
        return NULL;
    }

    fseek( fp, 0L, SEEK_END );
    size = ftell(fp);
    LOGI("%s video file size is %d", H264_FILE, size );
    ...
    while ( ipc->running && p_video_buf < p_end ) {
        ...
        if ( p_video_buf + frame_len < p_end ) {
            ...
            if ( ipc->param.video_cb )
                ipc->param.video_cb( (uint8_t *)p_video_buf, frame_len, !(type == 1), timestamp, 0 );
...
int GetFileSize( char *_pFileName)
{
    ...
    if( !fp ) {
        LOGE("fopen file %s error", _pFileName );
        return -1;
    }
...
void *sim_ipc_audio_task( void *param )
{
    ...
    int len = GetFileSize(AAC_FILE);
    if ( len <= 0 ) {
        LOGE("GetFileSize error");
        return NULL;
    }

    LOGI("%s audio file size:%d", AAC_FILE, len);
    FILE *fp = fopen( AAC_FILE, "r" );
    if( !fp ) {
        LOGE("open file %s error", AAC_FILE);
        return NULL;
    }
    ...
    while( ipc->running ) {
        ...
        if ( offset + 7 <= len ) {
            ...
            if ( offset + size + var.aac_frame_length <= len ) {
                if ( ipc->param.audio_cb ) {
                    count++;
                    interval = ((1024*1000.0)/aacfreq[fix.sampling_frequency_index]);
                    timeStamp = interval * count;
                    ipc->param.audio_cb( (uint8_t *)buf_ptr + offset, var.aac_frame_length, timeStamp, 0 );
                }
            }
...
static void *sim_ipc_motion_detect_task( void *arg )
{
    ...
    if ( !ipc->param.event_cb ) {
        return NULL;
    }

    for (;;) {
        sleep( 5 );
        ipc->param.event_cb( EVENT_MOTION_DETECTION, NULL, 0 );
        sleep( 8 );
        ipc->param.event_cb( EVENT_MOTION_DETECTION_DISAPEER, NULL, 0 );
    }
    return NULL;
}
...
static int sim_ipc_capture_picture( struct ipc_dev_t *ipc, char *path, char *file, int ch )
{
    sim_ipc_t *sim = (sim_ipc_t *) ipc->priv;
    char buf[512] = { 0 };
    ...
    sprintf( buf, "cp %s %s", JPG_FILE, file );
    LOGI("cmd : %s\n", buf );
    system( buf );
    if ( sim->param.event_cb ) {
        sim->param.event_cb( EVENT_CAPTURE_PICTURE_SUCCESS, file, 0 );
    }
    return 0;
}
